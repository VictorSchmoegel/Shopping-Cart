...
const addProduct = (botao, id) => {
  botao.addEventListener('click', async () => {
    saveCartID(id);
    const productId = await fetchProduct(id);
    const elementProduct = cartProduct;
    const elementRef = createCartProductElement(productId);
    elementProduct.appendChild(elementRef);
  });
};
